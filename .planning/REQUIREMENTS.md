...
- [x] **TRNS-05**: Retry logic with exponential backoff handles rate limits and transient subprocess failures
- [x] **TRNS-06**: Session ID captured from Claude output and reusable across commands via --resume flag
...
- [x] **OUT-03**: `--output-format` flag supports `json` (default) and `text` (human-readable)
- [x] **OUT-04**: Usage metadata includes token count and estimated cost per command invocation
...
- [x] **STATE-04**: Projects can be registered, listed, and removed via `openclawpack projects add/list/remove`
...
| CMD-05 | Phase 2.1 | Complete |
| CMD-06 | Phase 2 | Complete |
| CMD-07 | Phase 2.1 | Complete |
