...
<task type="auto">
  ...
  <action>
    ...
    **src/openclawpack/cli.py:**
    ...
    - Add a placeholder `status` command that prints `{"message": "not yet implemented"}` -- the placeholder ensures the CLI has at least one command for `--help` to display. NOTE: Wiring this stub to `get_project_summary()` is Phase 2 scope (CMD-04), not Phase 1. Plan 01-02 builds `get_project_summary()` but does NOT wire it to the CLI.
