...
<task type="auto">
  ...
  <action>
    ...
    **3. Create `src/openclawpack/commands/engine.py`:**
    ...
    - `async def run_gsd_command(self, command: str, prompt_args: str = "", answer_map: dict[str, str] | None = None) -> CommandResult` — the core method:
      a. Constructs prompt: `f"/{command}{' ' + prompt_args if prompt_args else ''}"`
