...
must_haves:
  ...
  key_links:
    ...
    - from: "src/openclawpack/cli.py"
      ...
      via: "CLI status command lazily imports and calls status_workflow()"
      pattern: "from openclawpack\\.commands\\.status import status_workflow"
    - from: "src/openclawpack/cli.py"
      to: "src/openclawpack/commands/new_project.py"
      via: "CLI new-project command lazily imports and calls new_project_workflow()"
      pattern: "from openclawpack\\.commands\\.new_project import new_project_workflow"
---
    ...
    **Create `src/openclawpack/commands/new_project.py`:**
    ...
    2. `async def new_project_workflow(idea: str, project_dir: str | None = None, verbose: bool = False, quiet: bool = False, timeout: float | None = None, answer_overrides: dict[str, str] | None = None) -> CommandResult`:
       ...
       - Call `await engine.run_gsd_command("gsd:new-project", prompt_override=prompt, answer_map=answer_map)`
         The engine's `run_gsd_command()` accepts `prompt_override: str | None = None` (defined in 02-01). When provided, the engine uses it directly instead of constructing `/{command} {prompt_args}`.
...
6. Verify cli.py wiring: `python -c "from openclawpack.commands.status import status_workflow; from openclawpack.commands.new_project import new_project_workflow"` — imports succeed, confirming CLI dispatch paths are valid
</verification>
