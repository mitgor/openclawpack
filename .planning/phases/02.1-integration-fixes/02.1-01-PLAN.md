...
**Bug 3 -- verbose/stderr support preparation:**
Add an optional `verbose` kwarg to `run()` (popped from kwargs alongside `can_use_tool` and `hooks`). When `verbose=True`, set `options.stderr` to a callback that prints to sys.stderr:
```python
...
```
This prepares client.py for Plan 02 to wire verbose through from the engine.
...
Run `pytest tests/test_transport/test_client.py -x -v` -- existing tests should still pass (they mock sdk_query so the wiring change is transparent to permissive mocks). Some tests may need adjustment in Task 2.
  ...
  <done>
`client.py` sets `can_use_tool` and `hooks` on `ClaudeAgentOptions` fields, wraps prompt as `AsyncIterable` when `can_use_tool` is set, and supports a `verbose` kwarg for `stderr` output. The `sdk_query()` call uses only `prompt` and `options` keyword arguments.
  </done>
</task>
