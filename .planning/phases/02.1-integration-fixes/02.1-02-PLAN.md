...
must_haves:
  truths:
    ...
    - "WorkflowEngine passes quiet flag through to ClaudeTransport.run() so SDK stderr output is suppressed"
  ...
  artifacts:
    - path: "src/openclawpack/commands/engine.py"
      provides: "Verbose and quiet forwarding to transport, build_hooks_dict() usage, TransportError catching"
  ...
  key_links:
    - from: "src/openclawpack/commands/engine.py"
      ...
      via: "verbose and quiet kwargs passed through to ClaudeTransport.run()"
      pattern: "run\\(.*verbose=|run\\(.*quiet="
    - from: "src/openclawpack/commands/engine.py"
...
<objective>
Wire verbose and quiet flags through the engine to the SDK, fix hooks structure in the engine, and add structured error propagation at the workflow boundary so that transport failures return JSON errors instead of tracebacks.

Purpose: Without error catching, users see raw Python tracebacks when Claude Code is missing. Without verbose/quiet forwarding, the `--verbose` and `--quiet` flags are accepted but ignored at the transport level. Without `build_hooks_dict()`, the engine passes a bare callable where the SDK expects `list[HookMatcher]`.

Output: All three workflow modules catch `TransportError` and return `CommandResult.error()`. The engine forwards `verbose` and `quiet` to `ClaudeTransport.run()` and uses `build_hooks_dict()` for correct hook structure. Tests validate all behaviors.
</objective>
...
<task type="auto">
  <name>Task 1: Fix engine.py -- verbose/quiet forwarding, build_hooks_dict(), and error catching</name>
...
**Change 2 -- Forward verbose and quiet flags to transport.run() (new):**
...
```
This passes `quiet=True` or `verbose=True` through to `ClaudeTransport.run()` which (from Plan 01) pops them and sets `options.stderr` accordingly (`None` for quiet, stderr callback for verbose). `quiet` takes precedence over `verbose` -- if both are set, the user asked for quiet so suppress output.
...
Run `pytest tests/test_commands/test_engine.py -x -v` -- existing tests pass (they mock ClaudeTransport so error catching is transparent). Some may need adjustment in Task 2.
  ...
  <done>
`WorkflowEngine` uses `build_hooks_dict()` for correct HookMatcher structure, forwards `verbose` and `quiet` to `ClaudeTransport.run()`, and catches `TransportError` to return `CommandResult.error()` instead of propagating exceptions.
  </done>
</task>
...
5. `test_quiet_forwarded_to_transport_run`: Create engine with `quiet=True`, run a command, capture `run()` kwargs, assert `captured_kwargs["quiet"] is True`.

6. `test_quiet_not_forwarded_when_false`: Create engine with `quiet=False`, run a command, assert `"quiet" not in captured_kwargs`.

7. `test_quiet_takes_precedence_over_verbose`: Create engine with `quiet=True, verbose=True`, run a command, capture `run()` kwargs, assert `captured_kwargs["quiet"] is True` and `"verbose" not in captured_kwargs`.
...
Verify no tracebacks: `python -c "import asyncio; from openclawpack.commands.new_project import new_project_workflow; r = asyncio.run(new_project_workflow('test')); print(r.success, r.errors)"` -- should print `False ['...']` (structured error), not a traceback. (This will hit CLINotFound or similar since Claude Code likely isn't configured for test.)
  ...
  <done>
All three workflow functions catch exceptions and return `CommandResult.error()`. Engine tests verify HookMatcher structure in hooks, verbose forwarding, quiet forwarding, quiet-over-verbose precedence, and TransportError handling. Workflow tests verify error-to-CommandResult wrapping. No raw tracebacks escape to the CLI.
  </done>
</task>
...
4. Grep `engine.py` for `verbose` -- confirms verbose flag forwarding
5. Grep `engine.py` for `quiet` -- confirms quiet flag forwarding
6. Grep `new_project.py` for `except` -- confirms error wrapping
7. Grep `plan_phase.py` for `except` -- confirms error wrapping
8. Grep `execute_phase.py` for `except` -- confirms error wrapping
</verification>
...
- `WorkflowEngine` forwards `quiet=True` to `ClaudeTransport.run()` when engine is quiet (suppresses SDK stderr)
- `WorkflowEngine` catches `TransportError` and returns `CommandResult.error()`
- Quiet takes precedence over verbose when both flags are set
- All three workflow functions have defensive error catching returning `CommandResult.error()`
- Tests verify HookMatcher structure, verbose forwarding, quiet forwarding, quiet-over-verbose precedence, error catching at engine level, and error wrapping at workflow level
